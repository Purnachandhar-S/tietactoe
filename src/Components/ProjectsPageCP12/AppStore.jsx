...
function AppStore() {
  const [activeStateId, setActiveStateId] = useState(tabsList[0].tabId);
  const [searchInput, setSearchInput] = useState("");

  // Filter apps based on the active category
  const filteredList = appsList.filter(
    (each) => each.category === activeStateId
  );

  // Handle search input change
  const onChangeInput = (event) => {
    const searchInputEntered = event.target.value;
    setSearchInput(searchInputEntered);
  };

  // Handle tab click
  const clickTabItem = (tabValue) => {
    setActiveStateId(tabValue);
  };

  // Filter apps based on search input
  const searchFilteredList = filteredList.filter((each) =>
  ...
  return (
    <div>
      ...
      <ul style={{ listStyle: "none", display: "flex" }}>
        {tabsList.map((tabDetails) => (
          <TabItem
            ...
            isActive={activeStateId === tabDetails.tabId}
          />
        ))}
      </ul>

      {/* Render apps, based on search or category */}
      <ul
        style={{
          listStyle: "none",
          display: "flex",
      ...
      >
        {searchInput
          ? searchFilteredList.map((appDetails) => (
              <AppItem key={appDetails.appId} appDetails={appDetails} />
            ))
          : filteredList.map((appDetails) => (
              <AppItem key={appDetails.appId} appDetails={appDetails} />
            ))}
      </ul>
    </div>
